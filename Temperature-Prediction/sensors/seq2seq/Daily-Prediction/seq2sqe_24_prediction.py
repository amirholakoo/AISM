...
input_seq_length = 24  # 2 hours past minutes
output_seq_length = 24  # Next 1 hour future minutes

lr = 1e-4
num_epochs = 2000
batch_size = 32
hidden_size = 100 # Reduced for stability
num_gru_layers = 1
grad_clip =1.0
scheduled_sampling_decay = 200
...
def load_data(device):
    period = 24 - 1
    df = pd.read_csv('Modified_Timeseries_Temperature_and_Humidity_1_Hour.csv')
    ...
    for time_seies_indx in range(df_array.shape[0]):
        time_series_i = df_array[time_seies_indx]
    ...
    df_array = df_array[:, period:, :]
    df_array = np.concatenate((df_array, lag_time_series), 2)
    # Lag: period=59 for 60-min lag minus 1
    #period = 23
    #lag = np.array([temperature[i - period] for i in range(period, len(temperature),60)])  # (23122,)
...
def split_data(data):
    ...
    train_end_indx = round(0.70 * num_timesteps)
    train_data = data[:, : train_end_indx]
    test_end_indx = train_end_indx + round(0.15 * num_timesteps)
...
class Encoder(nn.Module):
    def __init__(self, enc_feature_size, hidden_size, num_gru_layers, dropout):
        super().__init__()
        self.lstm = nn.LSTM(enc_feature_size, hidden_size, num_gru_layers, batch_first=True, dropout=dropout)
...
class DecoderBase(nn.Module):
    ...
    def forward(self, inputs, hidden, enc_outputs, teacher_force_prob=None):
        ...
        hidden_state = hidden  # initialize outside loop as (hidden, cell)
        for t in range(dec_output_seq_length):
            dec_output, hidden_state = self.run_single_recurrent_step(curr_input, hidden_state, enc_outputs)
            dec_output = torch.nan_to_num(dec_output, nan=0.0)
            outputs[:, t:t+1, :, :] = dec_output
            dec_output_sample = Seq2Seq.sample_from_output(dec_output)
            ...
            if not teacher_force:
                curr_input[:, :, self.target_indices] = dec_output_sample
        return outputs 
...
class DecoderVanilla(DecoderBase):
    def __init__(self, dec_feature_size, dec_target_size, hidden_size, 
        ...
        self.lstm = nn.LSTM(dec_feature_size, hidden_size, num_gru_layers, batch_first=True, dropout=dropout)
        self.out = layer_init(nn.Linear(hidden_size + dec_feature_size, dec_target_size * dist_size), is_sigma=True)
    
    def run_single_recurrent_step(self, inputs, hidden_state, enc_outputs):
...
class DecoderWithAttention(DecoderBase):
    def __init__(self, dec_feature_size, dec_target_size, hidden_size, 
        ...
        self.lstm = nn.LSTM(dec_feature_size + hidden_size, hidden_size, num_gru_layers, batch_first=True, dropout=dropout)
        self.out = layer_init(nn.Linear(hidden_size + hidden_size + dec_feature_size, dec_target_size * dist_size), is_sigma=True)

    def run_single_recurrent_step(self, inputs, hidden_state, enc_outputs):
        hidden, cell = hidden_state  # unpack
        # Attention using the last layer of hidden state
        weightings = self.attention_model(hidden[-1], enc_outputs)
        weighted_sum = torch.bmm(weightings.unsqueeze(1), enc_outputs)  # (batch, 1, hidden_size)
        
        # LSTM step
        lstm_input = torch.cat((inputs, weighted_sum), dim=2)
        output, (hidden, cell) = self.lstm(lstm_input, (hidden, cell))
        
        # Output layer
        output_combined = torch.cat((output, weighted_sum, inputs), dim=2)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs, teacher_force_prob=None):
        enc_outputs, hidden_state = self.encoder(enc_inputs)  # hidden_state = (hidden, cell)
        outputs = self.decoder(dec_inputs, hidden_state, enc_outputs, teacher_force_prob)
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    seq2seq.opt,        # your optimizer
    mode='min',         # reduce LR when val_loss stops decreasing
    factor=0.99,         # LR multiplied by this factor
    patience=10,        # wait 10 epochs before reducing      # prints a message when LR is reduced
...
for epoch in range(num_epochs):
    ...
    current_lr = seq2seq.opt.param_groups[0]['lr']
    print(f'Epoch {epoch+1} => Train loss: {train_loss:.5f}, Val: {val_loss:.5f}, LR: {current_lr:.6f}, Teach: {teacher_force_prob:.2f}, Took {(time() - start_t):.1f} s{"      (NEW BEST)" if new_best_val else ""}')
