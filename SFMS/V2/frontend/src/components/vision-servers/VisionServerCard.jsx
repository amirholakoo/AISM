...
import { Eye, Globe, Tag, Edit, Camera } from 'lucide-react';
  ...
  return servers.map((server) => (
    <Card key={server.id} className="p-4 hover:shadow-md transition-shadow duration-200">
      <div className="space-y-3">
        ...
        {/* منبع ویدیو */}
        <div className="flex justify-between items-center">
          <span className="text-sm font-medium text-gray-500">منبع ویدیو:</span>
          <div className="flex items-center gap-2">
            <Camera className="w-4 h-4 text-slate-400" />
            <span className="text-sm text-gray-700 text-right max-w-32 truncate" title={server.video_source}>
              {server.video_source === 'picamera' ? 'دوربین رزبری پای' : 
               server.video_source?.startsWith('rtsp://') ? `RTSP: ${server.video_source.split('//')[1]?.split('/')[0] || 'Stream'}` :
               server.video_source === 'webcam' ? 'Webcam' :
               server.video_source === '0' ? 'USB (0)' :
               server.video_source === '1' ? 'USB (1)' :
               server.video_source?.startsWith('/') ? 'فایل ویدیو' :
               server.video_source || 'نامشخص'}
            </span>
          </div>
        </div>
