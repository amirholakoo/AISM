...
export default function ShipmentSelectionInitialPage() {
  ...
  useEffect(() => {
    const loadShipments = async () => {
      try {
        ...
        await handleCopyDatabase(false); // بدون نمایش alert
      } catch (error) {
  ...
  const handleCopyDatabase = async (showAlerts = true) => {
    try {
      if (showAlerts) {
        setMessage("در حال دریافت دیتابیس از سرور خارجی...");
        setError(false);
      ...
      if (!remoteServerHealthy) {
        ...
        hasError = true;
        errorMessage = "سرور خارجی در دسترس نیست. از دیتابیس محلی استفاده می‌شود.";
      }
      ...
      if (sshTest) {
        ...
        if (copySuccess) {
          ...
          hasError = true;
          errorMessage = "خطا در کپی دیتابیس از سرور خارجی. از دیتابیس محلی استفاده می‌شود.";
        }
      } else {
        console.warn("⚠️ SSH connection failed, continuing with existing database");
        hasError = true;
        errorMessage = "اتصال به سرور خارجی برقرار نشد. از دیتابیس محلی استفاده می‌شود.";
      }
      ...
      if (data.success) {
        ...
        if (showAlerts) {
          if (hasError) {
            ...
            setMessage("دیتابیس با موفقیت دریافت شد و محموله‌ها به‌روزرسانی شدند!");
            setError(false);
          } else {
            setMessage(`محموله‌ها از دیتابیس محلی بارگذاری شدند (${shipmentsData.length} محموله)`);
            setError(false);
          }
        }
      } else {
      ...
      if (showAlerts) {
        setError(true);
        setMessage("خطا در دریافت دیتابیس از سرور خارجی. از دیتابیس محلی استفاده می‌شود.");
      }
    } finally {
      if (showAlerts) {
