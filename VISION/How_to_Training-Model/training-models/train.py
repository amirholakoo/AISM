...
def _patch_ultralytics_sppf():
    try:
        import inspect
        import ultralytics.nn.modules.block as block
        sig = inspect.signature(block.SPPF.__init__)
        if len(sig.parameters) <= 4:
            _orig_init = block.SPPF.__init__
            def _new_init(self, c1, c2, k=5, *args, **kwargs):
                _orig_init(self, c1, c2, k)
            block.SPPF.__init__ = _new_init
    except Exception:
        pass

_patch_ultralytics_sppf()

def _patch_ultralytics_conv():
    try:
        import ultralytics.nn.modules.conv as conv
        _orig_init = conv.Conv.__init__
        def _new_init(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
            if isinstance(g, bool):
                g = int(g)
            _orig_init(self, c1, c2, k, s, p, g, d, act)
        conv.Conv.__init__ = _new_init
    except Exception:
        pass

_patch_ultralytics_conv()
...
def train_and_export(dataset_path: str = "dataset", epochs: int = 100, size: str = "n"):
    print("\n" + "="*50)
    print(f"    YOLO26{size} TRAINING FOR RASPBERRY PI 5 + NCNN")
    print("="*50)
    ...
    # BEST MODEL PATH
    best_pt = Path(f"yolo26{size}_pi5/factory_detection/weights/best.pt")
    last_pt = Path(f"yolo26{size}_pi5/factory_detection/weights/last.pt")
...
def main():
    ...
    epochs = 50               # 50–100 usually enough for small datasets
