...
MODEL_DIR_NAME = "weights/best_50_v1.onnx"
...
os.environ["ANBAR_EXPOSURE_US"] = os.environ.get("ANBAR_EXPOSURE_US", "16000")
os.environ["ANBAR_ANALOG_GAIN"] = os.environ.get("ANBAR_ANALOG_GAIN", "7.5")
...
logging.basicConfig(
    level=logging.INFO,
    format="vision_id=anbar_khamir %(asctime)s [%(levelname)s] %(message)s",
    handlers=[
...
CONF_THRESH = 0.4        # Balanced confidence threshold
...
def _process_video():
    ...
    def write_run_log(message):
        ...
        run_log_file.write(f"{timestamp} device_id=anbar_khamir {message}\n")
        run_log_file.flush()

    report_data = {
        "device_id": "anbar_khamir",
