...
class VideoProcessor:
    ...
    def initialize_model(self, weights_path):
        """
        Initialize the ONNX model with given weights.
        This version is simplified for production and only supports .onnx files.
        
        Args:
            weights_path: Path to YOLO .onnx weights file
        ...
        try:
            ...
            logger.error(f"Failed to load ONNX model '{weights_path}': {e}", exc_info=True)
            return False
    ...
    def _run_onnx_inference(self, frame, model_input_size):
        """Preprocesses a frame and runs inference using the ONNX model."""
        # Pre-process the image
        input_img = cv2.resize(frame, (model_input_size, model_input_size))
