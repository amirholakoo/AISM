...
class Config:
    ...
    #MODEL_PATH =  "best_ncnn_model-yolo26-512-1-26"
    ...
    #VIDEO_INPUT_PATH = "video_source/pending/01.mp4"
    VIDEO_OUTPUT_PATH = ""
    VIDEO_CODEC = "mp4v"
    ...
    COUNT_DEBOUNCE_SEC = 0.7
    COUNT_DEBOUNCE_PX = 220
    COUNT_DEBOUNCE_IOU = 0.6
    SHOW_DISPLAY = True
...
class ObjectTracker:
    def __init__(self, config):
        ...
        self.count_debounce_sec = getattr(config, "COUNT_DEBOUNCE_SEC", 0.7)
        self.count_debounce_px = getattr(config, "COUNT_DEBOUNCE_PX", 220)
        self.count_debounce_iou = getattr(config, "COUNT_DEBOUNCE_IOU", 0.6)
        self.recent_counts = defaultdict(deque)
    ...
    def _bbox_iou(self, bbox_a, bbox_b):
        ...
        if union <= 0:
            return 0.0
        return inter / union
    def _is_duplicate_count(self, class_name, event_type, per_id, center_x, bbox, now_ts):
        if self.count_debounce_sec is None or self.count_debounce_sec <= 0:
            return False
        key = (class_name, event_type)
        entries = self.recent_counts[key]
        while entries and now_ts - entries[0]["ts"] > self.count_debounce_sec:
            entries.popleft()
        for entry in entries:
            if per_id is not None and entry.get("per_id") == per_id:
                return True
            if abs(center_x - entry["center_x"]) <= self.count_debounce_px:
                if self._bbox_iou(bbox, entry.get("bbox")) >= self.count_debounce_iou:
                    return True
        return False
    def _record_count(self, class_name, event_type, per_id, center_x, bbox, now_ts):
        key = (class_name, event_type)
        self.recent_counts[key].append({"ts": now_ts, "center_x": center_x, "bbox": bbox, "per_id": per_id})
    def _should_increment(self, class_name, event_type, per_id, center_x, bbox):
        now_ts = time.time()
        if self._is_duplicate_count(class_name, event_type, per_id, center_x, bbox, now_ts):
            return False
        self._record_count(class_name, event_type, per_id, center_x, bbox, now_ts)
        return True
    def update(self, detections, frame_index):
        ...
        for class_name, items in class_tracks.items():
            ...
            for item in frame_items:
                ...
                if st["x2"] < st["x1"] and not st["enter_counted"] and (st["x1"] - st["x2"] > 300):# and st["x1"] > self.entry_line_x:# 100
                    has_parallel = False
                    for other in moving_left_items:
                        if other["per_id"] != per_id and abs(center_x - other["center_x"]) <= 300:# 100
                            has_parallel = True
                            break
                    if class_name == "forklift":
                        if st["x1"] > self.entry_line_x:
                            ...
                            if self._should_increment(class_name, "enter", per_id, center_x, st["bbox"]):
                    ...
                    if class_name == "forklift":
                        if st["x2"] > self.entry_line_x:
                            ...
                            if not st["exit_counted"]:
                                if self._should_increment(class_name, "exit", per_id, center_x, st["bbox"]):
    ...
    def _write_output(self, frame_index):
        ...
        output_dict["total_entered"] = [{"class_name": name, "count": int(cnt)} for name, cnt in self.total_entered.items()]
        ...
        output_dict["device_id"] = "anbar_khamir_kordan"
