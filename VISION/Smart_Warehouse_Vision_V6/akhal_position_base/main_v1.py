...
class Config:
    ...
    #VIDEO_INPUT_PATH = "video_source/pending/05.mp4"
    VIDEO_OUTPUT_PATH = ""
    VIDEO_CODEC = "mp4v"
    ...
    QUEUE_SIZE = 32
    PROCESSING_QUEUE_SIZE = 128
    FPS_SMOOTHING_FRAMES = 8
...
class ProcessingWorker:
    def __init__(self, detector, maxsize=128):
    ...
    def _run(self):
        while not self.stopped or not self.input_queue.empty():
            try:
                frame_data = self.input_queue.get(timeout=0.2)
            except queue.Empty:
                continue
            ...
            while not self.stopped:
                try:
                    self.output_queue.put(result, timeout=0.2)
                    break
                except queue.Full:
                    continue
    ...
    def submit(self, frame_data):
        while not self.stopped:
            try:
                self.input_queue.put(frame_data, timeout=0.2)
                return True
            except queue.Full:
                continue
        return False
    ...
    def has_pending(self):
        return not self.output_queue.empty() or not self.input_queue.empty()
...
def run_simple():
    ...
    processor = ProcessingWorker(detector, config.PROCESSING_QUEUE_SIZE).start()
    tracker = ObjectTracker(config)
    annotator = FrameAnnotator(config)
    fps_calculator = FPSCalculator(config.FPS_SMOOTHING_FRAMES)
    video_reader.start()
    try:
        while video_reader.more() or processor.has_pending():
            frame_data = video_reader.read()
            if frame_data is None:
                result = processor.get_result(timeout=0.2)
            else:
                frame_number = frame_data["frame_number"]
                if frame_number % config.PROCESS_EVERY_N_FRAMES == 0:
                    processor.submit(frame_data)
            ...
            fps_value = fps_calculator.end_frame()
