...
class Config:
    ...
    #VIDEO_INPUT_PATH = "video_source/pending/25.mp4"
    VIDEO_OUTPUT_PATH = ""
    VIDEO_CODEC = "mp4v"
    ALLOWED_CLASSES = ["forklift", "paper-roll","akhal"]
    ...
    SHOW_DISPLAY = True
    DISPLAY_WINDOW_NAME = "khamir Counter"
    QUEUE_SIZE = 32
    FPS_SMOOTHING_FRAMES = 30
    PROCESS_EVERY_N_FRAMES = 1
    DETECTION_PIPELINE_DEPTH = 2
    DETECTION_WORKERS = 2
    ENTRY_LINE_X =1100# 300 #1600
    BBOX_ALPHA = 0.6
...
class ObjectTracker:
    ...
    def update(self, detections, frame_index):
        ...
        for class_name, items in class_tracks.items():
            ...
            # for item in frame_items:
            #     st = class_states[item["per_id"]]
            #     if st["x1"] > st["x2"] and (st["x1"] - st["x2"] > 100):
            #         moving_left_items.append(item)
            # for item in frame_items:
            #     per_id = item["per_id"]
            #     center_x = item["center_x"]
            ...
            #     if st["x1"] > st["x2"] and not st["exit_counted"] and (st["x1"] - st["x2"] > 100):
            #         has_parallel = False
            #         for other in moving_left_items:
            #             if other["per_id"] != per_id and abs(center_x - other["center_x"]) <= 100:
            #                 has_parallel = True
            ...
            #                     self.total_internal[class_name] += 1
            #                     st["internal_counted"] = True
            #     if st["x1"] < st["x2"] and not st["enter_counted"] and (st["x2"] - st["x1"] > 300):
            #         if class_name == "forklift":
            #             if st["x1"] > self.entry_line_x:
            #                 st["situation"] = "enter"
            #                 if not st["enter_counted"]:
    ...
    def _write_output(self, frame_index):
        ...
        output_dict["device_id"] = "anbar_khamir"
   ...
   # def _init_picamera2(self):
    #    try:
      ...
      #          main={"size": (1920, 1080), "format": "RGB888"},
                # Low resolution stream for LoF (optional)
                # lores={"size": (640, 480), "format": "YUV420"}
     #       )
    #        self.picam2.configure(config)
     #       # Manual controls for better lighting (optional)
   #         self.picam2.set_controls({
   #             "AwbMode": 1,           # Auto White Balance
      #          "ExposureTime": 8000,   # Fast shutter (microseconds) - same value used in this code
    #            "AnalogueGain": 4.0,
       #         "Brightness": 0.1,
        #        "Contrast": 1.2
        #    })
       #     self.picam2.start()
     #       time.sleep(2)  # Extra time for camera stabilization
    #        self.width = 1920
    #        self.height = 1080
    #        self.fps = 30.0
    #        self.total_frames = 0
