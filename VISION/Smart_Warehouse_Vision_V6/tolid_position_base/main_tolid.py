...
class Config:
    ...
    #VIDEO_INPUT_PATH = "video_source/pending/26.mp4"
    VIDEO_OUTPUT_PATH = ""
    VIDEO_CODEC = "mp4v"
    ALLOWED_CLASSES = ["forklift","paper-roll"]
    #ALLOWED_CLASSES = ["forklift", "sulfate", "pack", "fructose", "paper-roll","sude", "akd"]
    ...
    SHOW_DISPLAY = True
    DISPLAY_WINDOW_NAME = "TolidCounter"
    QUEUE_SIZE = 32
...
# ────────────────────────────────────────────────
# KalmanFilter, FPSCalculator, FrameAnnotator,
# ObjectDetector, ObjectTracker classes remain unchanged
# (omitted here for brevity – keep them as they were)
# ────────────────────────────────────────────────
...
class VideoReader:
    ...
    def _init_picamera2(self):
        try:
            ...
            # ────────────────────────────────────────────────────────────────
            # Force wider field of view by selecting a larger sensor output mode
            # Most common good values:
            #   Camera Module 3 → (2304, 1296) or (4608, 2592)
            #   Camera Module 2 → (1296, 972)  or (2592, 1944)
            # ────────────────────────────────────────────────────────────────
            ...
            config = self.picam2.create_preview_configuration(
                ...
                # This is the most important change → reduces zoom / cropping
                sensor={
                    'output_size': (2304, 1296)     # ← try this first
                    # Alternatives to experiment with:
                    # 'output_size': (1296, 972)
                    # 'output_size': (4608, 2592)   # full sensor (slower but widest)
                }
            )

            self.picam2.configure(config)
            self.picam2.start()
            time.sleep(1.8)  # longer wait → helps with stabilization on some modules
            ...
            print("PiCamera2 started → 1280×720 with wider FOV (sensor output 2304×1296)")
...
class FrameAnnotator:
    ...
    def _draw_info_overlay(self, frame, counts, fps):
        ...
        total_exit_count = counts.get("total_exit", 0)
        cv2.putText(frame, f"Total Exit: {total_exit_count}", (10, 95), self.config.FONT,
                    self.config.FONT_SCALE_MEDIUM, self.config.COLOR_TOTAL_TEXT, self.config.FONT_THICKNESS)
# ====================== Object Detector ======================
...
class ObjectTracker:
    ...
    def update(self, detections, frame_index):
        ...
        for class_name, items in class_tracks.items():
            ...
            for item in frame_items:
                ...
                if st["x2"] < st["x1"] and (st["x1"] - st["x2"] > 300):
                    moving_left_items.append(item)
            for item in frame_items:
                per_id = item["per_id"]
                center_x = item["center_x"]
                st = class_states[per_id]
                if st["x2"] < st["x1"] and not st["exit_counted"] and (st["x1"] - st["x2"] > 300):#100
                    has_parallel = False
                    for other in moving_left_items:
                        if other["per_id"] != per_id and abs(center_x - other["center_x"]) <= 300:
                            has_parallel = True
                            break
                ...
                if st["x2"] > st["x1"] and not st["enter_counted"] and (st["x2"] - st["x1"] > 300):#100
            ...
            # for item in frame_items:
            #     st = class_states[item["per_id"]]
            #     if st["x2"] > st["x1"] and (st["x2"] - st["x1"] > 100):
            #         moving_left_items.append(item)
            # for item in frame_items:
            #     per_id = item["per_id"]
            #     center_x = item["center_x"]
            ...
            #     if st["x2"] < st["x1"] and not st["enter_counted"] and (st["x1"] - st["x2"] > 100):
            #         has_parallel = False
            #         for other in moving_left_items:
            ...
            #                     self.total_internal[class_name] += 1
            #                     st["internal_counted"] = True
            #     if st["x2"] > st["x1"] and not st["exit_counted"] and (st["x2"] - st["x1"] > 300):
            #         if class_name == "forklift":
            #             if st["x2"] > self.entry_line_x:
            #                 st["situation"] = "Exit"
            #                 if not st["exit_counted"]:
        ...
        total_count = sum(int(cnt) for cnt in self.total_entered.values())
        total_exit_count = sum(int(cnt) for cnt in self.total_exit.values())
    ...
    def _write_output(self, frame_index):
        ...
        global LAST_DETECTED_OFFSET
        output_dict = {}
        final_class_name = False
        first_obj_timestamp = None
        last_obj_timestamp = None
        for class_name, items in self.class_states.items():
            if not class_name == "forklift" :
                final_class_name = class_name
            for k, v in items.items():
                if class_name != "forklift":
                    ts = v.get("timestamp")
                    if ts is not None:
                        if first_obj_timestamp is None or ts < first_obj_timestamp:
                            first_obj_timestamp = ts
                        if last_obj_timestamp is None or ts > last_obj_timestamp:
                            last_obj_timestamp = ts
        ...
        total_entered_count = sum(int(cnt) for cnt in self.total_entered.values())
        total_exit_count = sum(int(cnt) for cnt in self.total_exit.values())
        output_dict["total_entered"] =[{"class_name": name, "count": int(cnt)} for name, cnt in self.total_entered.items()]# total_entered_count
        output_dict["total"] =int(self.total_entered.get(final_class_name, 0))#akhalnt
        output_dict["total_exit"] =  [{"class_name": name, "count": int(cnt)} for name, cnt in self.total_exit.items()]
        output_dict["total_exit_by_class"] =[]# [{"class_name": name, "count": int(cnt)} for name, cnt in self.total_exit.items()]
        output_dict["total_internal"] = []#[{"class_name": name, "count": int(cnt)} for name, cnt in self.total_internal.items()]
        output_dict["device_id"] = "anbar_tolid"
        output_dict["class_name"] = final_class_name if final_class_name else "forklift"
        output_dict["first_obj_timestamp"] = first_obj_timestamp
        output_dict["last_obj_timestamp"] = last_obj_timestamp
    ...
    try:
        while video_reader.more():
            ...
            annotated = annotator.annotate_frame(
                frame, tracking_info["tracks"], tracking_info["counts"], fps_value
            )
